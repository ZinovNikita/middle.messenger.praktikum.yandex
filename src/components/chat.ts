...
export default class Chat extends Component{
    constructor(){
        super(chat_template,'div',{props:{className: 'chat-box'}});
    ...
    private setLogic(){
        ...
        this.$find(`#${this.$uid}-message-form > textarea`)?.addEventListener("input",(event:Event)=>{
            let msg:string = (<HTMLTextAreaElement>event.target).value;
            if(this.message !== msg)
                this.message = msg;
            //message validation
    ...
    private load(chat: obj){
        ...
        setTimeout(()=>{
            ...
            let msgs:obj[] = messages[chat.id as keyof typeof messages];
            this.messages = new Proxy(msgs,{
                get: (target:any, prop:string)=>{
                    if(prop==='push') this.data.messages = target.sort((a:obj,b:obj)=>{
                        if(a>b) return 1;
